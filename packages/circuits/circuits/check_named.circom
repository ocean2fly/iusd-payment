...
// Named check — proves ownership of stealth address
// Public: merkle_root, nullifier_hash, check_id, recipient_stealth_addr
// Private: nonce, spending_key, amount, target, version, params_hash, path_elements, path_indices
template CheckNamed() {
    signal input merkle_root;
    signal input nullifier_hash;
    signal input check_id;
    signal input recipient_stealth_addr;
    ...
    // Target must match public recipient stealth address
    target === recipient_stealth_addr;

    // Commitment (instrument=2 for check)
...
component main {public [merkle_root, nullifier_hash, check_id, recipient_stealth_addr]} = CheckNamed();
