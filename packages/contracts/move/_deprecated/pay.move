...
module ipay::pay {
    ...
    const PAYMENT_TYPE: u8 = 1; // PAYMENT_TYPE_TRANSFER
    ...
    const E_INVALID_PUBKEY: u64 = 110;
    ...
    struct PayPool has key {
        ...
        // Tables
        payments: Table<vector<u8>, Payment>,           // id -> order
        nullifiers: Table<vector<u8>, bool>,            // nullifier -> spent
        user_payments: Table<address, vector<vector<u8>>>, // user -> ids
        ...
        pending_payments: vector<vector<u8>>,             // ids with STATUS_CREATED
    }
    ...
    #[event]
    struct PaymentStatusChangedEvent has drop, store {
        id: vector<u8>,
        from_status: u8,
        to_status: u8,
        action: u8,
    ...
    /// Initialize the send pool (legacy - stores at deployer address)
    /// For new deployments, use create_pool_object() via Factory
    public entry fun initialize(
        ...
        assert!(!exists<PayPool>(deployer_addr), error::already_exists(E_ALREADY_INITIALIZED));

        let constructor_ref = object::create_object(deployer_addr, false);
        let extend_ref = object::generate_extend_ref(&constructor_ref);

        move_to(deployer, PayPool {
            extend_ref,
            iusd_fa,
            owner: deployer_addr,
            treasury,
    ...
    /// Create a new PayPool as an Object (Factory pattern)
    /// Each call creates a new pool at a unique address
    public fun create_pool_object(
        owner: &signer,
        iusd_fa: Object<Metadata>,
        treasury: address,
        fee_bps: u64,
        ...
        // Create a new Object for this pool
        let constructor_ref = object::create_object(owner_addr, false);
        let extend_ref = object::generate_extend_ref(&constructor_ref);
        let object_signer = object::generate_signer(&constructor_ref);
        
        // Store PayPool in the Object (not at owner address)
        move_to(&object_signer, PayPool {
            extend_ref,
            iusd_fa,
            owner: owner_addr,
            treasury,
        ...
        // Initialize ViewingKeyBackups storage in the Object
        move_to(&object_signer, ViewingKeyBackups {
            backups: table::new(),
        });
        
        // Return the Object
        object::object_from_constructor_ref<PayPool>(&constructor_ref)
    }
    ...
    public entry fun deposit(
        ...
        let pool = borrow_global_mut<PayPool>(pool_addr);

        // Validations
        assert!(!table::contains(&pool.payments, id), error::already_exists(E_NOT_INITIALIZED));
        ...
        // Store order
        table::add(&mut pool.payments, id, payment);
        ...
        // Update stats
        pool.total_payments = pool.total_payments + 1;
        pool.total_volume = pool.total_volume + amount;
        
        // Add to pending_payments for Relayer to process
        vector::push_back(&mut pool.pending_payments, id);

        event::emit(PaymentCreatedEvent { id, recipient_cooked });
    }
    ...
    public entry fun spend(
        ...
        assert!(table::contains(&pool.payments, id), error::not_found(E_PAYMENT_NOT_FOUND));
        let payment = table::borrow_mut(&mut pool.payments, id);
        
        assert!(payment.status == STATUS_CREATED, error::invalid_state(E_INVALID_TRANSITION));
        common::check_not_expired(payment.expires_at);
        ...
        payment.status = STATUS_PENDING_CLAIM;
        
        // Remove from pending_payments
        let (found, index) = vector::index_of(&pool.pending_payments, &id);
        if (found) {
            vector::remove(&mut pool.pending_payments, index);
        };
    ...
    public entry fun spend_pending_register(
        ...
        let from_status = payment.status;
        payment.status = STATUS_PENDING_REGISTER;
    ...
    /// REQUIRES: Caller must be an authorized relayer
    public entry fun activate_payment(
        relayer: &signer,
        pool_addr: address,
        id: vector<u8>,
        ...
        assert!(payment.status == STATUS_PENDING_REGISTER, error::invalid_state(E_INVALID_TRANSITION));
        common::check_not_expired(payment.expires_at);
        
        // Update key_for_recipient with new encrypted version
        payment.key_for_recipient = new_key_for_recipient;
    ...
    public entry fun claim(
        ...
        assert!(vector::length(&id) > 0, error::not_found(E_PAYMENT_NOT_FOUND));
        ...
        // Verify claim key
        assert!(key_hash == payment.claim_key_hash, error::permission_denied(E_INVALID_KEY));
        ...
        payment.claimed_by = recipient_addr;
        payment.claimed_at = now;

        event::emit(PaymentClaimedEvent { id, claim_key_hash: key_hash });
        event::emit(PaymentStatusChangedEvent {
            id,
            from_status,
    ...
    public entry fun revoke(
        ...
        assert!(
            payment.status == STATUS_PENDING_CLAIM || payment.status == STATUS_PENDING_REGISTER,
            error::invalid_state(E_INVALID_TRANSITION)
        );
        common::check_not_expired(payment.expires_at);
        assert!(key_hash == payment.refund_key_hash, error::permission_denied(E_INVALID_KEY));
        ...
        event::emit(PaymentRevokedEvent { id, refund_key_hash: key_hash });
        event::emit(PaymentStatusChangedEvent {
            id,
            from_status,
    ...
    public entry fun refund(
        ...
        let pool = borrow_global<PayPool>(pool_addr);
        let recipient_addr = signer::address_of(recipient);
        
        // Validate payment exists and is confirmed
        assert!(table::contains(&pool.payments, id), error::not_found(E_PAYMENT_NOT_FOUND));
        let payment = table::borrow(&pool.payments, id);
        ...
        assert!(payment.claimed_by == recipient_addr, error::permission_denied(E_NOT_RECIPIENT));
        ...
        let pool_mut = borrow_global_mut<PayPool>(pool_addr);
        let payment_mut = table::borrow_mut(&mut pool_mut.payments, id);
        let from_status = payment_mut.status;
        payment_mut.status = STATUS_REFUNDED;
    ...
    public entry fun expire(
        ...
        let payment = table::borrow_mut(&mut pool.payments, id);
        assert!(payment.status == STATUS_PENDING_CLAIM, error::invalid_state(E_INVALID_TRANSITION));
        assert!(common::is_expired(payment.expires_at), error::invalid_state(E_PAYMENT_EXPIRED));
    ...
    /// Update admin public key (for payment encryption/decryption)
    public entry fun update_admin_pubkey(
        owner: &signer,
        pool_addr: address,
        new_admin_pubkey: vector<u8>,
    ) acquires PayPool {
        let pool = borrow_global_mut<PayPool>(pool_addr);
        assert!(signer::address_of(owner) == pool.owner, error::permission_denied(E_NOT_AUTHORIZED));
        assert!(vector::length(&new_admin_pubkey) == 65, error::invalid_argument(E_INVALID_PUBKEY));
        pool.admin_pubkey = new_admin_pubkey;
    }

    /// Check if address is an authorized relayer
    #[view]
    public fun is_relayer(pool_addr: address, addr: address): bool acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
    ...
    public entry fun set_viewing_pubkey_for(
        ...
        viewing_pubkey: vector<u8>,
        viewing_pubkey_type: u8,
    ) acquires PayPool, SponsoredViewingKeys {
        let pool = borrow_global<PayPool>(pool_addr);
        let relayer_addr = signer::address_of(relayer);
    ...
    #[view]
    public fun get_payments_by_cooked(
        pool_addr: address,
        cooked_address: vector<u8>,
        include_dummies: bool,
        offset: u64,
        limit: u64,
    ): vector<Payment> acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        ...
        while (i < len && count < limit) {
            ...
            if (table::contains(&pool.payments, id)) {
                let payment = *table::borrow(&pool.payments, id);
                vector::push_back(&mut result, payment);
                count = count + 1;
            };
            i = i + 1;
        };
    ...
    public fun get_payment(
        pool_addr: address,
        id: vector<u8>,
    ): Option<Payment> acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        if (table::contains(&pool.payments, id)) {
            option::some(*table::borrow(&pool.payments, id))
        } else {
            option::none()
        }
    }
    ...
    #[view]
    public fun get_payment_count(
        pool_addr: address,
        cooked_address: vector<u8>,
    ): u64 acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        if (table::contains(&pool.cooked_index, cooked_address)) {
    ...
    /// Get pool stats
    #[view]
    public fun get_pool_stats(pool_addr: address): (u64, u64, u64) acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        (pool.total_payments, pool.total_volume, pool.total_fees)
    }

    /// Get pool config
    #[view]
    public fun get_pool_config(pool_addr: address): (address, address, u64, u64) acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
    ...
    #[view]
    public fun get_pending_payments(pool_addr: address): vector<vector<u8>> acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
    ...
    #[view]
    public fun get_pending_payments_count(pool_addr: address): u64 acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        vector::length(&pool.pending_payments)
    }

    /// Get order by claim_key_hash
    /// Returns (status, created_at, expires_at) or abort if not found
    #[view]
    public fun get_payment_by_claim_key(
        pool_addr: address,
        claim_key_hash: vector<u8>,
    ): (u8, u64, u64) acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        let id = find_order_by_claim_key(pool, &claim_key_hash);
        assert!(vector::length(&id) > 0, error::not_found(E_PAYMENT_NOT_FOUND));
        
        let payment = table::borrow(&pool.payments, id);
        (payment.status, payment.created_at, payment.expires_at)
    }
    ...
    fun find_order_by_claim_key(pool: &PayPool, key_hash: &vector<u8>): vector<u8> {
    ...
    fun find_order_by_refund_key(pool: &PayPool, key_hash: &vector<u8>): vector<u8> {
    ...
    fun create_dummy_order(seed: u64, cooked: vector<u8>): Payment {
        Payment {
            id: common::generate_dummy_id(seed, @0x0),
    ...
    public fun get_pending_register_ids(pool_addr: address): vector<vector<u8>> acquires PayPool {
        let pool = borrow_global<PayPool>(pool_addr);
        let result = vector::empty<vector<u8>>();
        
        // Iterate through pending_payments and find PENDING_REGISTER ones
        let pending = &pool.pending_payments;
        let i = 0;
        ...
        while (i < len) {
            ...
            if (table::contains(&pool.payments, *id)) {
                let payment = table::borrow(&pool.payments, *id);
                if (payment.status == STATUS_PENDING_REGISTER) {
