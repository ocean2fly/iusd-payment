/// iPay Common - Shared constants, types, and utilities
/// 
/// All payment types share these definitions for consistency.
module ipay::common {
    use std::error;
    use std::vector;
    ...
    const PAYMENT_TYPE_TRANSFER: u8 = 1;
    const PAYMENT_TYPE_REQUEST: u8 = 2;
    const PAYMENT_TYPE_SUBSCRIPTION: u8 = 3;
    const PAYMENT_TYPE_GIFT: u8 = 4;
    const PAYMENT_TYPE_MULTI_APPROVAL: u8 = 5;
    ...
    struct PaymentSummary has copy, drop, store {
        id: vector<u8>,
        payment_type: u8,
        status: u8,
        created_at: u64,
    ...
    public fun payment_type_transfer(): u8 { PAYMENT_TYPE_TRANSFER }
    /// @deprecated Use payment_type_transfer() instead
    public fun payment_type_send(): u8 { PAYMENT_TYPE_TRANSFER }
    public fun payment_type_request(): u8 { PAYMENT_TYPE_REQUEST }
    public fun payment_type_subscription(): u8 { PAYMENT_TYPE_SUBSCRIPTION }
    public fun payment_type_gift(): u8 { PAYMENT_TYPE_GIFT }
    public fun payment_type_multi_approval(): u8 { PAYMENT_TYPE_MULTI_APPROVAL }
    ...
    public fun e_order_not_found(): u64 { E_PAYMENT_NOT_FOUND }
    public fun e_order_expired(): u64 { E_PAYMENT_EXPIRED }
    ...
    public fun check_not_expired(expires_at: u64) {
        assert!(!is_expired(expires_at), error::invalid_state(E_PAYMENT_EXPIRED));
    }
